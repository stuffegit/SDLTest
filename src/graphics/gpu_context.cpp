...
GPUContext::GPUContext(SDL_Window* window) {
  ...
  // avoids SDL trying to fall back to Direct3D or Metal with a shader it can't use.
  ...
  // VSYNC ties frame presentation to the monitor's refresh rate (typically 60 or 144 Hz).
  // It prevents screen tearing by only swapping the image on a vertical blank.
  // Note: SDL_SubmitGPUCommandBuffer returns immediately (GPU work is async), so
  // a separate CPU-side frame limiter is still needed to avoid spinning at 9000+ FPS.
